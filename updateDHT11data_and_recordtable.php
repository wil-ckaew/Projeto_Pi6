<?php
  ...
  if (!empty($_POST)) {
    ...
    $soil_humidity = $_POST['soil_humidity'];
    ...
    $sql = "UPDATE esp32_table_dht11_leds_update SET temperature = ?, humidity = ?, soil_humidity = ?, status_read_sensor_dht11 = ?, time = ?, date = ? WHERE id = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($temperature,$humidity, $soil_humidity,$status_read_sensor_dht11,$tm,$dt,$id));
    Database::disconnect();
    ...
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		...
		$sql = "INSERT INTO esp32_table_dht11_leds_record (id,board,temperature,humidity,soil_humidity,status_read_sensor_dht11,LED_01,LED_02,time,date) values(?, ?, ?, ?, ?, ?,?, ?, ?, ?)";
		$q = $pdo->prepare($sql);
		$q->execute(array($id_key,$board,$temperature,$humidity, $soil_humidity,$status_read_sensor_dht11,$led_01,$led_02,$tm,$dt));
